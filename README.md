...
│       ├── 18-view-review.js    ← 描画：週次レビューのオーバーレイ（REVIEW/applyReviewAct）
...
- 画面上部の「書き出し」ボタンで、その時点の全データを JSON ファイルとしてダウンロード
  できます（`nextaction-gtd-YYYY-MM-DD.json`）。
...
- 週次レビューの各節の行に、その場で状態を変えるボタンを出します。押しても
  **オーバーレイは閉じません**。その場で状態を変えて描き直すだけなので、
  条件を満たさなくなった行はその節から消えます（＝押すたびに残り件数が減ります）。
  従来からの「開く」ボタン（クリックでオーバーレイを閉じてパネルを開く）は
  そのまま残っています。
- ボタンは4種類、節ごとに宣言的に割り当てています（`src/js/18-view-review.js`
  の `REVIEW` 各エントリの `acts`）。
  ...
  | 期限切れを拾う | 完了 / いつかへ |
  | 待ちを督促する | 完了 / 次へ |
  | 止まっているプロジェクトを動かす | （なし。行がプロジェクトのため） |
  | 停滞したアクションを見直す | 完了 / いつかへ / 今日 |
  | いつかリストを棚卸しする | 完了 / 次へ / 今日 |

- 「今日」ボタンは `state` を `next` にしたうえで「今日やる」印
  （`flagged`）を立てます。「今日やる」印の表示条件が `state==="next"` を含むため、
  いつか等の項目に印だけ付けても「今日やる」ビューには出てこないからです。
- 状態遷移そのものは純関数 `applyReviewAct(it, act)` に切り出してあり、
  `test/review-act.test.js` はこれを直接テストしています。
- レビュー行内の操作は取り消し（Undo）の対象です。完了・状態変更は元に戻せるべき、
  という Undo 全体の方針（「作る・並べ替えるだけ」の操作は対象外、それ以外は対象）
  と一貫させています。

## 複数行ペースト

- 収集欄（`#capIn`）に改行を含むテキストを貼り付けると、1行1項目として
  まとめて収集トレイへ追加されます（議事録やメモから複数行を一気に、という用途）。
- 分割は改行（`\r\n` / `\n`）ごと。各行は前後の空白を取り除き、空行は捨てます。
...
- 設定パネルの「古い完了項目の整理」欄から、90日より前の完了項目を
  **JSON として書き出してから削除**できます（`nextaction-gtd-archive-YYYY-MM-DD.json`）。
...
- 入力欄（テキストボックス等）にフォーカスがあるときの `Cmd+Z` / `Ctrl+Z` は
  ブラウザ標準のテキスト取り消しに任せ、このツール側は何もしません。
