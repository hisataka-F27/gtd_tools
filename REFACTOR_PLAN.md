...
- ~~（Phase 5 で発見）`addProjectAction`（`#pAdd`、プロジェクト内の行動追加）は、`#ctxNew` と違って
  ...
  するのが自然な改善だと考えられる。~~
  → **リファクタ完了後に修正済み。** `addProjectAction` の末尾に `$("#pAdd").focus()` を追加した。
  等価性テスト（`phase4-actions-equivalence.test.js`）には `PADD_REFOCUS` として意図的な挙動変更で
  あることを置換ルールで明記してあり、それ以外の差分は引き続き検出される。
